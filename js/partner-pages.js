(() => {
  ...
  async function enrichBookings(items){try{const data=await request("/deals"),deals=[data,data?.items,data?.deals,data?.data,data?.data?.items,data?.result?.items].find(Array.isArray)||[];items.forEach(item=>{item._deal=deals.find(deal=>Number(deal.id)===Number(item.deal_id))||{};});}catch{items.forEach(item=>{item._deal={};});}return items;}
