(() => {
  ...
  function bookingRow(item) {
    ...
    const deal=item._deal||{},image=dealImage({...deal,image:deal.image||item.deal_image}),title=deal.title||item.deal_title||`Deal #${item.deal_id||"—"}`;
    return `<tr><td><strong>#${escapeHtml(item.id)}</strong><small>${formatDate(item.created_at,true)}</small><button class="sd-view-booking" type="button" data-booking="${escapeAttribute(item.id)}">Bekijk boeking</button></td><td><div class="sd-booking-deal">${image?`<img src="${escapeAttribute(image)}" alt="">`:""}<div><strong>${escapeHtml(title)}</strong><small>${escapeHtml(item.guests||1)} gast(en)</small></div></div></td><td>${escapeHtml(dates||"—")}</td><td><strong>${money(item.total_amount)}</strong><small>${escapeHtml((item.currency||"EUR").toUpperCase())}</small></td><td>${badge(status)}</td></tr>`;
  }
  async function enrichBookings(items){let partnerDeals=[],publicDeals=[];try{const data=await request("/deals");partnerDeals=[data,data?.items,data?.deals,data?.data,data?.data?.items,data?.result?.items].find(Array.isArray)||[];}catch{}try{const response=await fetch("https://xgrq-dkge-tace.n7e.xano.io/api:seasondeals-public/deals",{headers:{Accept:"application/json"}}),data=await response.json();publicDeals=[data,data?.items,data?.deals,data?.data,data?.data?.items].find(Array.isArray)||[];}catch{}items.forEach(item=>{const partner=partnerDeals.find(deal=>Number(deal.id)===Number(item.deal_id))||{},published=publicDeals.find(deal=>Number(deal.id)===Number(item.deal_id))||{};item._deal={...published,...partner,images:partner.images?.length?partner.images:published.images,image:partner.image||published.image};});return items;}
  function dealImage(deal){const value=deal.image||deal.main_image||deal.featured_image||deal.images?.[0],url=typeof value==="string"?value:(value?.url||value?.path||"");return url&&!url.startsWith("http")?`https://xgrq-dkge-tace.n7e.xano.io${url.startsWith("/")?"":"/"}${url}`:url;}
  function openBooking(item){if(!item)return;const modal=document.getElementById("sd-booking-modal"),deal=item._deal||{},customer=item.customer||{},voucher=[item.voucher_code||item.code,item.voucher_status?`(${label(item.voucher_status)})`:""].filter(Boolean).join(" ");modal.innerHTML=`<div class="sd-booking-dialog"><button class="sd-modal-close" type="button" aria-label="Sluiten">×</button><span class="sd-page-eyebrow">Boeking #${escapeHtml(item.id)}</span><h2>${escapeHtml(deal.title||item.deal_title||`Deal #${item.deal_id||"—"}`)}</h2><div class="sd-booking-details">${detail("Gast",item.customer_name||customer.name||"Nog niet beschikbaar")}${detail("E-mail",item.customer_email||customer.email||"Nog niet beschikbaar")}${detail("Telefoon",item.customer_phone||customer.phone||"Nog niet beschikbaar")}${detail("Verblijf",`${formatDate(item.checkin)} – ${formatDate(item.checkout)}`)}${detail("Aantal gasten",`${item.guests||1} gast(en)`)}${detail("Bedrag",money(item.total_amount))}${detail("Betaalstatus",label(item.payment_status||item.status||"pending"))}${detail("Voucher",voucher||"Nog niet beschikbaar")}${detail("Opmerkingen",item.notes||item.special_requests||"Geen opmerkingen")}</div></div>`;modal.classList.add("is-open");modal.setAttribute("aria-hidden","false");modal.querySelector(".sd-modal-close").addEventListener("click",closeBooking);modal.addEventListener("click",event=>{if(event.target===modal)closeBooking();},{once:true});}
  function detail(name,value){return `<div><span>${escapeHtml(name)}</span><strong>${escapeHtml(value)}</strong></div>`;}function closeBooking(){const modal=document.getElementById("sd-booking-modal");modal?.classList.remove("is-open");modal?.setAttribute("aria-hidden","true");}
  ...
  async function loadIntegrations(){
    setLoading();
    try{
      const data=normalize(await request("/integrations")),items=integrationItems(data);
      content().innerHTML=items.length?`<div class="sd-integration-grid">${items.map(integrationCard).join("")}</div>`:empty("Nog geen integratie gekoppeld","SeasonDeals kan eerst een testintegratie voor jouw hotel activeren.");
      document.querySelectorAll("[data-integration-keys]").forEach(button=>button.addEventListener("click",()=>loadApiKeys(button.dataset.integrationKeys)));
    }catch(error){showPageError(error);}
  }
  function integrationCard(item){
    const provider=item.provider||item.integration_provider||{},status=item.status||"pending",environment=item.environment||"test";
    return `<section class="sd-integration-card"><div class="sd-integration-head"><div><span class="sd-page-eyebrow">${escapeHtml(provider.name||item.provider_name||"API-integratie")}</span><h2>${escapeHtml(item.name||provider.name||`Integratie #${item.id}`)}</h2></div>${badge(status)}</div><div class="sd-integration-facts">${detail("Omgeving",label(environment))}${detail("Synchronisatie",label(item.sync_direction||"bidirectional"))}${detail("Laatste succesvolle sync",formatDate(item.last_success_at||item.last_sync_at,true))}${detail("Extern hotel-ID",item.external_hotel_id||"Nog niet ingesteld")}</div>${item.last_error_message?`<div class="sd-integration-error">${escapeHtml(item.last_error_message)}</div>`:""}<div class="sd-integration-actions"><button class="sd-primary-button" type="button" data-integration-keys="${escapeAttribute(item.id)}">API-sleutels beheren</button></div><div id="sd-integration-keys-${escapeAttribute(item.id)}"></div></section>`;
  }
  async function loadApiKeys(integrationId){
    const target=document.getElementById(`sd-integration-keys-${integrationId}`);if(!target)return;target.innerHTML='<div class="sd-page-loading">Sleutels laden…</div>';
    try{const data=normalize(await request(`/integrations/${encodeURIComponent(integrationId)}/api-keys`)),items=integrationItems(data);target.innerHTML=`<div class="sd-key-panel"><div class="sd-key-panel-head"><h3>API-sleutels</h3>${isOwner()?`<button class="sd-secondary-button" type="button" data-create-key="${escapeAttribute(integrationId)}">Nieuwe testsleutel</button>`:""}</div>${items.length?`<div class="sd-key-list">${items.map(item=>keyRow(integrationId,item)).join("")}</div>`:'<p class="sd-key-empty">Er zijn nog geen API-sleutels.</p>'}</div>`;target.querySelector("[data-create-key]")?.addEventListener("click",()=>createApiKey(integrationId));target.querySelectorAll("[data-revoke-key]").forEach(button=>button.addEventListener("click",()=>revokeApiKey(integrationId,button.dataset.revokeKey)));}catch(error){target.innerHTML=`<div class="sd-integration-error">${escapeHtml(error.message)}</div>`;}
  }
  function keyRow(integrationId,item){const revoked=Boolean(item.revoked_at)||item.is_active===false;return `<div class="sd-key-row"><div><strong>${escapeHtml(item.name||"API-sleutel")}</strong><code>${escapeHtml(item.key_prefix||item.prefix||"sd_test_")}••••${escapeHtml(item.key_last4||item.last4||"")}</code><small>${escapeHtml(label(item.environment||"test"))} · laatst gebruikt ${formatDate(item.last_used_at,true)}</small></div>${revoked?badge("revoked"):`<button class="sd-key-revoke" type="button" data-revoke-key="${escapeAttribute(item.id)}">Intrekken</button>`}</div>`;}
  async function createApiKey(integrationId){
    const name=prompt("Naam voor deze testsleutel:","Testkoppeling");if(!name?.trim())return;
    try{const result=normalize(await request(`/integrations/${encodeURIComponent(integrationId)}/api-keys`,{method:"POST",body:{name:name.trim(),environment:"test",scopes:{"deals.read":true,"deals.write":true,"inventory.write":true,"bookings.read":true}}}));const secret=result.api_key||result.key||result.secret||result.full_key;if(!secret)throw new Error("De sleutel is aangemaakt, maar de eenmalige sleutel ontbreekt in de response.");showApiKey(secret);await loadApiKeys(integrationId);}catch(error){showMessage(error.message,"error");}
  }
  async function revokeApiKey(integrationId,keyId){if(!confirm("Deze API-sleutel definitief intrekken? De gekoppelde applicatie verliest direct toegang."))return;try{await request(`/integrations/${encodeURIComponent(integrationId)}/api-keys/${encodeURIComponent(keyId)}/revoke`,{method:"POST"});showMessage("API-sleutel is ingetrokken.","info");await loadApiKeys(integrationId);}catch(error){showMessage(error.message,"error");}}
  function showApiKey(secret){const modal=document.createElement("div");modal.className="sd-booking-modal is-open";modal.innerHTML=`<div class="sd-booking-dialog sd-key-dialog"><button class="sd-modal-close" type="button" aria-label="Sluiten">×</button><span class="sd-page-eyebrow">Eenmalig zichtbaar</span><h2>Bewaar deze testsleutel nu</h2><p>Na het sluiten kan de volledige sleutel niet opnieuw worden bekeken.</p><div class="sd-secret-row"><code>${escapeHtml(secret)}</code><button class="sd-primary-button" type="button">Kopiëren</button></div></div>`;document.body.append(modal);const close=()=>modal.remove();modal.querySelector(".sd-modal-close").addEventListener("click",close);modal.querySelector(".sd-primary-button").addEventListener("click",async event=>{try{await navigator.clipboard.writeText(secret);event.currentTarget.textContent="Gekopieerd";}catch{event.currentTarget.textContent="Selecteer handmatig";}});}
  function isOwner(){return ["owner","hotel_owner"].includes(String(user.role||"").toLowerCase());}
  function normalize(value){if(typeof value!=="string")return value;try{return JSON.parse(value);}catch{return value;}}
  function integrationItems(data){return [data,normalize(data?.items),normalize(data?.data),normalize(data?.data?.items),normalize(data?.result?.items)].find(Array.isArray)||[];}
