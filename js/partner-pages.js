(() => {
  ...
  function bookingRow(item) {
    ...
    return `<tr><td><strong>#${escapeHtml(item.id)}</strong><small>${formatDate(item.created_at,true)}</small></td><td><div class="sd-booking-deal">${image?`<img src="${escapeAttribute(image)}" alt="">`:""}<div><strong>${escapeHtml(title)}</strong><small>${escapeHtml(item.guests||1)} gast(en)</small></div></div></td><td>${escapeHtml(dates||"—")}</td><td><strong>${money(item.total_amount)}</strong><small>${escapeHtml((item.currency||"EUR").toUpperCase())}</small></td><td>${badge(status)}</td><td><button class="sd-view-booking" type="button" data-booking="${escapeAttribute(item.id)}">Bekijk boeking</button></td></tr>`;
  }
  ...
  function dealImage(deal){const value=deal.image||deal.main_image||deal.featured_image||deal.images?.[0];return typeof value==="string"?value:(value?.url||"");}
