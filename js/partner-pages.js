(() => {
  ...
  async function enrichBookings(items){const cache=new Map();await Promise.all(items.map(async item=>{if(!item.deal_id)return;try{if(!cache.has(item.deal_id))cache.set(item.deal_id,request(`/deals/${encodeURIComponent(item.deal_id)}`));const data=await cache.get(item.deal_id);item._deal=data?.deal||data;}catch{item._deal={};}}));return items;}
