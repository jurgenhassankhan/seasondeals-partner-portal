(() => {
  ...
  function getRemaining(item) { const direct=number(item.remaining_inventory??item.available_quantity); if(direct!==null)return Math.max(0,direct); const inventory=number(item.inventory); return inventory===null?null:Math.max(0,inventory-(number(item.sold_quantity)||0)); }
